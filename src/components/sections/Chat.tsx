...
const Chat: FC<ChatProps> = ({ isCollapse, toggleCollapse }) => {
  ...
  const {
    state: { socket, form, room, messages },
    dispatch,
  } = useStore();
      ...
      if (messages[0].type === "MESSAGE") {
        toast.info(
          <div className="flex items-center gap-2">
            ...
            <div className="rounded-full bg-theme px-4 py-1 text-white">
              {messages[0].content}
            </div>
          </div>,
          {
        ...
        toast.info(
          <div className="flex gap-1 self-center italic">
            <div className="max-w-[10ch] truncate font-bold text-self">
              {messages[0].playerName}
            </div>
          ...
          <div className="flex flex-1 flex-col-reverse gap-2 overflow-y-scroll border border-white/10 p-4 backdrop-blur-3xl">
            {messages.map((message, index) =>
              message.type === "NOTIFICATION" ? (
                ...
                <Message
                  key={index}
                  isSelf={message.playerName === form.playerName}
                  data={message}
                />
              )
            )}
